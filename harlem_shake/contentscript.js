...
// Creates a clickable element which will make the shaking stop when clicked.
function buildStopElement() {
  var stop = document.createElement('div');
  stop.innerText = "STOP SHAKING!!"
  stop.onclick = stopShake;
  return stop;
}
...
if (foundMatch()) {
  ...
  // Build the element that makes the page stop shaking and attach it to the DOM.
  var stop = buildStopElement();
  var node = document.body.childNodes.length > 0 ?
    document.body.childNodes[0] :
    null;
  document.body.insertBefore(stop, node);
